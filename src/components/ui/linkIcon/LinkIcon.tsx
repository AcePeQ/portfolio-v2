...
function LinkIcon({ path, children }: LinkIconProps) {
  return (
    <a target="_blank" className="w-10 h-10 flex items-center justify-center rounded-full border-2 border-white-darker bg-grey-normal 
    ...
    hover:text-white-dark-hover focus:text-white-dark-hover active:text-white-dark-active" href={path}>
      <span className="w-6 h-6 flex items-center justify-center [&>svg]:w-full [&>svg]:h-full">
        {children}
      </span>
    </a>
  )
